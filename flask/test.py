...
def journals_vis(journals, dates, years_range):
	...
	i = 0
	for j in journals:
		if j == (journal_year[i][0]): #journal name
			#print(str(j) + ' == ' + str(journal_year[i][0]))
	...
	#### USE JOURNALSTOTALDICT FOR SORTING VIS MOST TO LEAST######
...
pub_data, range_info = journals_vis(journals, dates, years_range)
print(pub_data)
